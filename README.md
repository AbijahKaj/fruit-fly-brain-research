...
- **Looming avoidance.** LC4/LPLC2 fitted in the same way: selectivity ~1.0 for approaching objects, nothing for gratings, receding or translating ones. In the scene the cruising fly turns 45–78° away from objects 4–8° off its heading.
- **Rotation vs translation.** The HS cells receive only ipsilateral input in this graph, so they were fitted as the classic bidirectional HS (progressive up, regressive below rest) on eye input recorded from the scene itself, and the trainer now runs the pooling cells at the browser's operating point. The drum is followed in all four directions, forward flight no longer reads as rotation, and the fly crosses the pillar course without collisions. Details in [`train/README.md`](train/README.md) and [`app/README.md`](app/README.md).
